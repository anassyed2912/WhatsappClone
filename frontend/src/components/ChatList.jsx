...
export default function ChatList({ conversations, onSelect, active }) {
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter(c =>
      (c.name || c.wa_id || '').toLowerCase().includes(term) ||
      (c.lastMessage?.body || '').toLowerCase().includes(term)
    );
      ...
      <div className="chat-list">
        {filteredConversations.map(c => (
          <div
            ...
            style={{
              background: active === c.wa_id ? '#f7fff9' : 'transparent'
            }}
          >
            <div className="avatar">
              {(c.name || c.wa_id || '').slice(0, 1)}
            </div>
